...
\begin{document}
	...
	\subsection{Technologies}
			\begin{itemize}
				...
				\\
				\\
				For the presentation layer we will use IONIC and Cordova which enables us to deploy to android, IOS and web. This is to make the application available on multiple platforms which is important
				...
				\\
				\\
				For the Service layer we will use Java Enterprise Editions' REST API, this is for improved scalability since the client and server are loosely coupled.
 				
			
				\item Persistence Layer
				\\
				\\
				For the Persistence layer we will use Java Persistence API(JPA), for accessing, managing and persisting data between data transfer objects. This allows us to focus more on the object model than the actual SQL, enables us to save time by generating/updating SQL queries and convert the results to our model classes. For the Database layer we will use JDBC, which offers a connection to our postgreSQL database, this works well since our persistence layer is generating SQL queries for postgreSQL, allowing us to manage and manipulate the database.
	
				
				\item Business Layer
				\\
				\\
				For the business layer we will be using Java Enterprise Edition to incoorperate methods and for coding the business logic.
