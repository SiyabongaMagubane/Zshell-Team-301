...
\begin{document}
	...
	Constraints are a result of design decisions that are fixed and may not be altered. Constraints may be intentional or unintentional often provided by stakeholders committed toward the project.
 	...
 	We will be implementing the microservices architectural design pattern.
 		...
 		\subsection{Concerns}
 			\begin{itemize}
 				...
 				\bigskip
 				\\
 				Complexity is of main concern to the microservices architecture. Due to all the independent services, programmers need to carefully handle requests between modules. Calls experiencing latency can also make thing more complex to handle. Coordination within the architecture may not be straightforward increasing the chance of failure between services.
 			\end{itemize}
 			\begin{itemize}
 				\item Unsafe Distributed Communication
 				\bigskip
 				\\
 				Each service runs independently, communicating over the network, increasing the likelihood that something will go wrong. Transaction safety is also of concern.
 			\end{itemize}
 			
 	\section{Integration Requirements}
 	Concerned with securely coordinated sharing of processes and data between modules and applications.
 	
 	\pagebreak
 	
 	\section{Modules Selected}
 		\subsection{Navigation}
 			...
 			The external interface requirements are concerned with the communication between the NavUP system and external components.
 				\subsection{User Inter}
